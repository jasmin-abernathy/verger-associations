...
(() => {
 const toggle=document.querySelector('[data-verger-accessibility-toggle]');if(!toggle)return;
 const key='verger-accessible-mode';
 const apply=enabled=>{document.documentElement.dataset.vergerAccessible=String(enabled);toggle.setAttribute('aria-pressed',String(enabled));toggle.textContent=enabled?'Version standard':'Version accessible'};
 let saved=false;try{saved=localStorage.getItem(key)==='true'}catch(_){}apply(saved);
 toggle.addEventListener('click',()=>{const enabled=document.documentElement.dataset.vergerAccessible!=='true';apply(enabled);try{localStorage.setItem(key,String(enabled))}catch(_){}});
})();
...
(() => {
 ...
 document.querySelectorAll('[data-site-context]').forEach(label=>{label.textContent=ref==='prestadmin'?'Prestadmin × Le Potager · version pilote 0.2':label.dataset.contextDefault});
 if(ref!=='direct'){
  document.querySelectorAll('a[href]').forEach(link=>{
   const raw=link.getAttribute('href');if(!raw||raw.startsWith('#')||/^(mailto:|tel:|javascript:)/i.test(raw))return;
   const url=new URL(raw,location.href);if(url.origin!==location.origin||url.searchParams.has('ref'))return;
   url.searchParams.set('ref',ref);link.href=url.pathname+url.search+url.hash;
  });
 }
})();
